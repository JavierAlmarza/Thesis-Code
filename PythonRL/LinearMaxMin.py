...
class MaxMinMaxSolver:
    def __init__(self, X, burn_in=50, lr_Y=1e-3, lr_ab=1e-3, lr_lambda=1e-4, device='cpu'):
        """
        Args:
    ...
    def get_a(self):
        return 0.1 * torch.tanh(self.raw_a)

    def get_b(self):
        return torch.tanh(self.raw_b)

    def get_lambda(self):
        return 10.0 * torch.tanh(self.raw_lambda)
...
if __name__ == "__main__":
    ...
    solver = MaxMinMaxSolver(X, burn_in=burn_in, lr_Y=1e-2, lr_ab=1e-2, lr_lambda=1e-2)
    solver.train(num_steps=1000, verbose=True)
