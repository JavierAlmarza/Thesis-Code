...
def Estimate(X_sim):
    # Suppose X_sim is your simulated price paths, shape (N, T)
    # We'll flatten to get all observations
    X_flat = X_sim.flatten()  

    mu = np.mean(X_flat)  # sample mean
    phi = 0.1             # mean reversion parameter from your model

    # Compute central moments m1,...,m15
    moments = []
    for k in range(1, 16):
        m_k = np.mean((X_flat - mu)**k)
        moments.append(m_k)

    # Taylor expansion for E[1/X]
    E_invX = 1/mu
    for k, m_k in enumerate(moments):
        E_invX += (-1)**(k+1) * m_k / mu**(k+1)  # m1 is moments[0]

    # Estimate E[R_oil] using the linear approximation
    E_Roil = phi * (mu * E_invX - 1)

    print("Estimated E[R_oil] (15-term Taylor):", E_Roil)
