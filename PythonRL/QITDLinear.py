...
class MaxMinMaxSolver:
    ...
    def qitd_step(self):
        ...
        u = torch.cat([
            ...
            self.get_a().view(1),
            self.get_b().view(1),
            self.get_lambda().view(1)
        ]).detach().requires_grad_(True)

        # current split
        Y_n = u[:self.T]
        a_n, b_n, lam_n = u[self.T:]

        # current loss + gradient
        loss_n, G_n = self.loss_and_grad(u)

        eta = self.eta

        # proposed step
        def trial_u(eta):
            return u - eta * (self.B @ G_n)

        # --- line search with double inequality ---
        while eta > self.eps * self.eta:
            u_try = trial_u(eta)

            Y_new = u_try[:self.T]
            a_new, b_new, lam_new = u_try[self.T:]

            # three losses
            L_theta_new_y_n = self.loss_full(Y_new, a_n, b_n, lam_n)
            L_both_new      = self.loss_full(Y_new, a_new, b_new, lam_new)
            L_theta_n_z_new = self.loss_full(Y_n,  a_new, b_new, lam_new)

            if (L_theta_new_y_n <= L_both_new) and (L_both_new <= L_theta_n_z_new):
                break
        ...
        eta = max(eta, 1e-4)
        if eta <= 1e-4:
            print('Eta reached minimum')
        self.eta = min((1 + self.beta) * eta, self.eta_max)

        # --- accept step ---
        u_new = trial_u(eta)

        # --- secant update ---
        loss_np1, G_np1 = self.loss_and_grad(u_new)

        s = self.apply_J(G_np1) - self.B @ G_n
        denom = torch.dot(G_n, s)

        if torch.abs(denom) > 1e-12:
            alpha = torch.dot(s, s) / denom
            alpha = torch.sign(alpha) * min(abs(alpha), 1.0)
            self.B = self.B + (alpha / torch.dot(s, s)) * torch.outer(s, s)

        # --- unpack ---
        with torch.no_grad():
            self.Y.copy_(u_new[:self.T])
            self.raw_a.copy_(torch.atanh(2 * u_new[self.T]))
            self.raw_b.copy_(torch.atanh(u_new[self.T+1]))
            self.raw_lambda.copy_(torch.atanh(u_new[self.T+2] / 125.0))
