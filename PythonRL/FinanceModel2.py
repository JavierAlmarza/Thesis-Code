...
def make_env(): # Vectorize environment for parallel PPO training
    return mF.PortfolioEnv2(R, q=q, k=k, include_action=include_action, include_vol=include_vol)
...
if UsesSCA:
    ...
    modelFin = PPO(policy, env , learning_rate=learning_rate, use_sde=use_sde, policy_kwargs=dict(net_arch=[64, 64],log_std_init=-1.0), ent_coef=0.00, gamma=gamma1, gae_lambda=0.9, clip_range=0.25, n_epochs=10,  batch_size=125, n_steps=250, verbose=1)


# Add Callback for plotting during learning

callback = mF.FinModelCallback(plot_freq=375, saveName = "training_Plot2.png")
...
if boolTest:
    R2 = mF.generate_cccmultivarch(1, T)
...
for i in range(T):
    a, _ = modelFin.predict(obs, deterministic=True)
    waction[i] = a[0]
...
fig.tight_layout()
fig.savefig("testing_plot2.png")  # saves to current working directory
