...
class HessianSolver:
    def __init__(self, X, cell, lmbda=10.0, eta_z_m=0.01, eta_y_m=0.01, eta_z_i=0.1, eta_y_i=0.1,device='cpu',split=False):
        ...
        self.eta_z_init = eta_z_i
        self.eta_y_init = eta_y_i
    ...
    def update_step(self, eta_z,eta_y,grad_min):
        ...
        while (not accept) and (eta_y + eta_z) > self.eta_y_min + self.eta_z_min:
            ...
            with torch.no_grad():
                #ratio = torch.linalg.norm(gw) / torch.linalg.norm(term_correction)
                ratio = torch.linalg.norm(gw)
                #print(ratio.item())
            ...
            if split:
                ...
                with torch.no_grad():
                    ratio = torch.linalg.norm(Lww) / torch.linalg.norm(term_cov)
                    #print(torch.linalg.norm(Lww).item())
                #print(ratio.item())
                block_inner = Lww - term_cov
                H_11 = torch.eye(2, device=self.device) - eta_z * block_inner
                H_12 = 0
            H_21 = eta_y * Lyw
            scale_H22 = 1.0 + eta_y * alpha
            ...
            w_old = w_current.detach()
            w_new = w_old + dw
            y_old = self.Y.detach()
            y_new = y_old + dy

            def compute_L_at(w_val, y_val):
                Z_val, _ = self.forward_Z(w_val)
                
                mse_val = ((self.X - y_val)**2).mean()
                Z_norm_val = Z_val - Z_val.mean()
                corr_val = (Z_norm_val * y_val).mean()
                
                return mse_val + self.lmbda * corr_val

            L_old_w_old = L.detach()
            L_new_w_old = compute_L_at(w_new, y_old)
            L_new_w_new = compute_L_at(w_new, y_new)
            
            accept_y = (L_new_w_new <= L_new_w_old)                
            accept_z = (L_new_w_new >= L_old_w_old ) | (gy.norm().item() > grad_min)
            accept = (accept_z & accept_y).item()
            if not accept:
                if not accept_z:
                    eta_z = 0.5 * eta_z
                else:
        ...
        return L.item(), mse.item(), corr.item(), gw.norm().item(), gy.norm().item(), norm_dw.item(), norm_dy.item(), accept
    ...
    def train(self, steps=100):
        ...
        while not_done:
            i = i+1
            L, mse, corr, nGw, nGy, ndw, ndy, accept = self.update_step(self.eta_z_init, self.eta_y_init, grad_min)
            not_done = accept & (i < steps)
            not_done = not_done & (nGy > grad_min)
            if i % 10 == 0:
...
if __name__ == "__main__":
    ...
    eta_y_i = T * 0.1
    split = False
    solver = HessianSolver(X, cell, lmbda=lmbda, eta_z_m=eta_z_m, eta_y_m=eta_y_m,eta_z_i=eta_z_i, eta_y_i=eta_y_i, split=split)
    print(" ")
    print(f"Starting training with parameters Lambda: {lmbda:.4f}, eta_z_i: {eta_z_i:.2f}, eta_y_i: {eta_y_i:.2f}")
    print(" ")
